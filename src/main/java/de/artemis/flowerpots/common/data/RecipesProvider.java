...
public class RecipesProvider extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildCraftingRecipes(@NotNull Consumer<FinishedRecipe> consumer) {
        ...
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(ModBlocks.TALL_UNFIRED_FLOWER_POT.get()), ModBlocks.TALL_FLOWER_POT.get().asItem(), 0.35F, 200).unlockedBy("has_tall_unfired_flower_pot", has(ModBlocks.TALL_UNFIRED_FLOWER_POT.get())).save(consumer);
        ShapedRecipeBuilder.shaped(ModBlocks.TALL_UNFIRED_FLOWER_POT.get()).define('A', Items.CLAY_BALL).pattern("A A").pattern("A A").pattern(" A ").unlockedBy("has_clay_ball", has(Items.CLAY_BALL)).save(consumer);
